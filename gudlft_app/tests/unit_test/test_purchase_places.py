...
def test_booking_no_more_than_places_available_in_competition(app, first_club_test, first_competition_test, second_competition_test, third_competition_test):
    email = first_club_test['email']
    places_required = 12
    assert places_required
    assert places_required > 0
    ...
    assert places_required > int(third_competition_test["number_of_places"])
    ...
    assert_template(
        ...
        email=email,
        data=dict(places=str(places_required), club=first_club_test['name'], competition=third_competition_test['name']),
        club=first_club_test,
        competitions=[first_competition_test, second_competition_test, third_competition_test]
    )
...
def test_booking_with_enough_points(app, second_club_test, first_competition_test, second_competition_test, third_competition_test):
    ...
    assert places_required <= 12
    assert places_required <= int(second_club_test["points"])
    assert places_required <= int(first_competition_test['number_of_places'])
    ...
    modified_first_competition_test["number_of_places"] = int(first_competition_test['number_of_places']) - places_required
